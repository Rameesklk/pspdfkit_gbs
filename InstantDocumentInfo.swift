...
public class InstantDocumentInfo: NSObject {
    ...
    let url: URL
    let jwt: String
    let tokens: [String] // Updated to be an array of strings

    @objc public init(serverURL: URL, url: URL, jwt: String, tokens: [String]) {
...
extension InstantDocumentInfo {
    enum JSONKeys: String {
        case serverUrl, url, documentId, jwt, tokens
    }
    ...
    convenience init?(json: Any) {
        guard let dictionary = json as? [String: Any],
            ...
            let jwt = dictionary[JSONKeys.jwt.rawValue] as? String,
            let tokens = dictionary[JSONKeys.tokens.rawValue] as? [String] else {
                return nil
        }

        self.init(serverURL: serverURL, url: url, jwt: jwt, tokens: tokens)
    }
}
